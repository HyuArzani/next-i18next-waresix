...
import { lngPathCorrector, subpathIsRequired, lngPathCorrectorIgnoreAs } from '../utils'
  ...
  wrappedMethods.forEach((method) => {
    Router[method] = (path, as, options) => {
      ...
      if (subpathIsRequired(config, i18n.languages[0])) {
        const fixerFunc = typeof as !== 'undefined' ? lngPathCorrector : lngPathCorrectorIgnoreAs 
        const { as: correctedAs, href: correctedHref } = fixerFunc(
